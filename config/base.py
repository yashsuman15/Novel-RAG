...
class BaseConfig(BaseSettings):
    ...
    data_dir: Path = Field(
        default=Path("data"),
        description="Data directory for PDFs and vector store"
    )
    ...
    batch_size: int = Field(
        ...
        ge=100,
        le=10000,
        description="Batch size for vector store insertions"
    )
    ...
    llm_model: str = Field(
        ...
        description="Primary LLM for answer generation"
    )
    
    llm_lite_model: str = Field(
        default="claude-sonnet-4-5",
        description="Lightweight LLM for query expansion"
    )
    ...
    retrieval_total_chunks: int = Field(
        ...
        ge=1,
        le=100,
        description="Total chunks to retrieve before reranking"
    )
    ...
    retrieval_rerank_top_k: int = Field(
        ...
        ge=1,
        le=50,
        description="Top K chunks after reranking"
    )
    ...
    def validate_rerank_top_k(cls, v, info):
        """Ensure rerank_top_k <= total_chunks."""
        total = info.data.get("retrieval_total_chunks", 20)
        if v > total:
            raise ValueError(
                f"retrieval_rerank_top_k ({v}) cannot exceed "
                f"retrieval_total_chunks ({total})"
            )
        return v
